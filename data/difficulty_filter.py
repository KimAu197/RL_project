...
def _build_filter_config(filter_cfg_dict: dict[str, Any], mode: str) -> DifficultyFilterConfig:
    return DifficultyFilterConfig(
...
def _build_generation_config(gen_cfg_dict: dict[str, Any]) -> tuple[GenerationConfigLite, int]:
    ...
    return gen_cfg, int(gen_cfg_dict.get("batch_size", 1))


def _build_reward_config(cfg: dict[str, Any], mode: str) -> RewardConfig:
    reward_cfg = cfg.get("reward", {})
    return RewardConfig(
        spider_root=cfg["data"]["spider_root"],
        mode=mode,
        timeout_s=float(reward_cfg.get("timeout_s", 5.0)),
        match_reward=float(reward_cfg.get("match_reward", 1.0)),
        validity_bonus=float(reward_cfg.get("validity_bonus", 0.0)),
        sketch_bonus=float(reward_cfg.get("sketch_bonus", 0.0)),
        no_sql_penalty=float(reward_cfg.get("no_sql_penalty", 0.0)),
    )


def _build_prompts(records: list[SpiderRecord], tokenizer, mode: str) -> list[str]:
    prompts = []
    for rec in records:
...
def _generate_completions_in_chunks(
    model,
    tokenizer,
    ...
    completions_nested: list[list[str]] = [[] for _ in batch_prompts]
    remaining = samples
    while remaining > 0:
        call_samples = min(samples_per_call, remaining)
        call_cfg = GenerationConfigLite(
            max_new_tokens=gen_cfg.max_new_tokens,
            temperature=gen_cfg.temperature,
            top_p=gen_cfg.top_p,
            do_sample=gen_cfg.do_sample,
            num_return_sequences=call_samples,
            repetition_penalty=gen_cfg.repetition_penalty,
        )
        call_nested = generate_batched(model, tokenizer, batch_prompts, call_cfg, batch_size=batch_size)
        for completions, new_completions in zip(completions_nested, call_nested):
            completions.extend(new_completions)
        remaining -= call_samples
    return completions_nested
...
def run_filter(config_path: str, cli_overrides: dict[str, Any]) -> int:
    ...
    filter_cfg = _build_filter_config(filter_cfg_dict, mode)
    ...
    samples_per_call = int(filter_cfg_dict.get("samples_per_call", 1))
    if samples_per_call <= 0:
        raise ValueError("difficulty_filter.samples_per_call must be positive")
    ...
    prompts = _build_prompts(records, tokenizer, mode)
    gen_cfg, batch_size = _build_generation_config(cfg.get("generation", {}))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--samples-per-call", type=int, default=None, help="sampled completions per generate() call")
